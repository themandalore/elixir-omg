defmodule OmiseGOWatcher.Challenger.CoreTest do
  ...
  test "creates a challenge for an exit", %{transactions: transactions} do
    utxo_exit = utxo_position(blknum: 1, txindex: 0, oindex: 0)
    challenging_tx = hd(transactions)

    expected_cutxopos = utxo_position(blknum: 2, txindex: 1, oindex: 0) |> UtxoPosition.encode()
    ...
    expected_cutxopos = utxo_position(blknum: 2, txindex: 2, oindex: 1)  |> UtxoPosition.encode()

    %Challenge{cutxopos: ^expected_cutxopos, eutxoindex: 1} =
      Core.create_challenge(challenging_tx, transactions, utxo_exit)

    utxo_exit = utxo_position(blknum: 1, txindex: 0, oindex: 1)
