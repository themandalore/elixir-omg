defmodule OmiseGOWatcher.BlockGetter do
  ...
  def consume_block(%{execables: execables, number: blknum} = block) do
    # TODO add check in UtxoDB after deposit handle correctly
    state_exec = for %Core.Execable{payload: to_exec} <- execables, do: apply(OmiseGO.API.State, :exec, to_exec)
